...
public class AvatarBubbleHandler : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // 核心安全拦截：确保组件齐全，且没有处于 Missing 状态
        if (animator == null || attachTarget == null) return;

        // 【关键保护】：确保底层 Animator 拥有合法的有效 Avatar。如果没有，直接跳出，拒绝执行
        ...
        if (bone == null)
        {
            ...
            if (bone == null) return; // 如果在该人形骨骼上找不到对应的骨骼点（比如模型不标准），也拦截
        }
        ...
        if (animator.GetBool("isBigScreen"))
        {
        ...
        else if (!shouldBeActive && originalParent != null && attachTarget.transform.parent != originalParent)
        {
    ...
    {
        return animator != null && animator.avatar != null && animator.GetBool("isDragging");
    }
