...
public class FoodEntry
{
    ...
    [Range(-3f, 3f)] public float minPitch = 1;
    [Range(-3f, 3f)] public float maxPitch = 1;
}
...
public class AvatarFoodController : MonoBehaviour
{
    ...
    public float verticalVelocityToPitch = 0.25f;
    ...
    public KeyCode testSpawnKey = KeyCode.F;
    public KeyCode testInteractKey = KeyCode.E;
    public KeyCode testDespawnKey = KeyCode.R;
    ...
    void LateUpdate()
    {
        ...
        if (debugMode)
        {
            if (Input.GetKeyDown(testSpawnKey) && foods.Count > 0)
            {
                Debug.Log($"<color=yellow>[FoodSystem] 按下 {testSpawnKey}，生成食物 (Index 0)</color>");
                SpawnByIndex(0);
            }

            if (Input.GetKeyDown(testInteractKey) && HasActive())
            {
                Debug.Log($"<color=yellow>[FoodSystem] 按下 {testInteractKey}，触发交互</color>");
                PlayInteract();
            }

            if (Input.GetKeyDown(testDespawnKey) && HasActive())
            {
                Debug.Log($"<color=yellow>[FoodSystem] 按下 {testDespawnKey}，销毁食物</color>");
                DespawnActive();
            }
        }
    }
    ...
    void HeadInteractCheck()
    {
        ...
        {
            Debug.Log("<color=green>[FoodSystem] 触发交互！</color>");
        ...
        if (root != currentAvatarRoot)
        {
            ...
            if (found != animator)
            {
                ...
                else
                {
                    head = null;
                    // 兜底：如果不是标准人形，尝试按名字找头
                    if (root != null) head = FindDeepChild(root.transform, "Head");
                }
                UpdateDepthFromHead();
            }
        }
    ...
    void OnDrawGizmos()
    {
        ...
        var bone = animator.GetBoneTransform(UnityEngine.HumanBodyBones.Head);
    ...
    public void EnableFeature() => SetFeatureEnabled(true);
    public void DisableFeature() => SetFeatureEnabled(true);
    public void ToggleFeature() => SetFeatureEnabled(true);
}