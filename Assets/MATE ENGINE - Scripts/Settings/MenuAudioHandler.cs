...
public class MenuAudioHandler : MonoBehaviour
{
    ...
    private IEnumerator MenuMonitor()
    {
        while (true)
        {
            ...
            {
                if (audioSource != null && !audioSource.gameObject.activeSelf) audioSource.gameObject.SetActive(true);
                if (!wasMenuOpenLastFrame) PlaySound(openMenuSounds, openMenuVolume);
                disableTimer = 0f;
            }
            else
            {
                if (wasMenuOpenLastFrame)
                {
                    ...
                    PlaySound(closeMenuSounds, closeMenuVolume);
                }
    ...
    private void SetupUIListeners()
    {
        ...
        foreach (var button in GetComponentsInChildren<Button>(true))
            if (button.GetComponent<ButtonLinker>() == null)
                button.onClick.AddListener(() => PlaySound(buttonSounds, buttonVolume));

        foreach (var toggle in GetComponentsInChildren<Toggle>(true))
            toggle.onValueChanged.AddListener((_) => PlaySound(toggleSounds, toggleVolume));
        ...
        foreach (var dropdown in GetComponentsInChildren<Dropdown>(true))
            dropdown.onValueChanged.AddListener((_) => PlaySound(dropdownSounds, dropdownVolume));
    }
    ...
    private void AddSliderEvents(Slider slider)
    {
        ...
        pointerDown.callback.AddListener((_) =>
        {
            if (!activeSliders.Contains(slider))
            {
                ...
                PlaySound(sliderSounds, sliderVolume);
            }
        });
    ...
    private void PlaySound(List<AudioClip> clips, float volume)
    {
    ...
    {
        if (audioSource != null && !audioSource.gameObject.activeSelf) audioSource.gameObject.SetActive(true);
        PlaySound(openMenuSounds, openMenuVolume);
    }
    ...
    {
        if (audioSource != null && !audioSource.gameObject.activeSelf) audioSource.gameObject.SetActive(true);
        PlaySound(buttonSounds, buttonVolume);
    }
}
