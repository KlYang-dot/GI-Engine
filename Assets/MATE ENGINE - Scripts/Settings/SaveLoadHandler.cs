...
public class SaveLoadHandler : MonoBehaviour
{
    ...
    private string BaseDir
    {
        get
        {
            // 获取运行目录下的 GIEngine 文件夹
            string runDir = System.AppDomain.CurrentDomain.BaseDirectory;
            string giEngineDir = Path.Combine(runDir, "GIEngine");
            return giEngineDir;
        }
    }
    ...
    private void Awake()
    {
        ...
        // 调试输出
        Debug.Log("[SaveLoadHandler] 数据目录: " + BaseDir);
        Debug.Log("[SaveLoadHandler] 完整文件目录: " + FilePath);
    ...
    public void LoadFromDisk()
    {
        ...
        {
            Debug.Log("[SaveLoadHandler] Settings file not found, creating new: " + FilePath);
            data = new SettingsData();
        }
        MigrateAfterLoad();
    }
