...
namespace Utils
{
    public static partial class TrayIcon
    {
...
#if UNITY_EDITOR
            // 如果是在编辑器里反复点击运行，并且检测到上一次运行的残存句柄或状态还活着
            if (_init && (messageWindowHandle != IntPtr.Zero || hIcon != IntPtr.Zero))
            {
                Debug.LogWarning("[MateEngine] 检测到 Unity 6 历史运行残留句柄，正在自动执行强制热重置...");
                CleanupResources();
                _init = false; // 强行解锁静态标记
            }
#endif
        ...
        private static bool CreateMessageWindow()
        {
            ...
            if (classAtom == 0)
            {
                // 针对 Unity 6 窗口类名可能也存在残留的特殊兼容处理
                if (Marshal.GetLastWin32Error() == 1410) // 1410 = ERROR_CLASS_ALREADY_EXISTS
                {
                    WinAPI.UnregisterClass(windowClassName, hInstance);
                    classAtom = WinAPI.RegisterClassEx(ref wc);
                }
        ...
        private static void CleanupResources()
        {
            ...
            if (messageWindowHandle != IntPtr.Zero)
            {
                // 无论 _init 状态如何，只要底层句柄在，就强行从 Windows 任务栏注销图标
                WinAPI.Shell_NotifyIcon(NIM_DELETE, ref notifyIconData);
            }
            ...
            if (hInstance != IntPtr.Zero && !string.IsNullOrEmpty(windowClassName))
            {
                WinAPI.UnregisterClass(windowClassName, hInstance);
            }
