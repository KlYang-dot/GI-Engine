...
namespace VRM
{
    public class VRMFirstPerson : MonoBehaviour
    {
        ...
        void OnDestroy()
        {
            // 在最新的 Unity 6 引擎运行环境下，为了彻底避免主线程强行物理销毁动态资产
            // 与 Native 层的底层异步垃圾回收（GC）发生二次释放/死锁冲突，我们不再在销毁时显式调用 Object.Destroy()。
            // 而是直接解绑引用、清理列表，让 Unity 引擎自带的底层 GC 在下一帧通过平滑安全的方式回收它们。
            if (m_headlessMeshes != null)
            {
